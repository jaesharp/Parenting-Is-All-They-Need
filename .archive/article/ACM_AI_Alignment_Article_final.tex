...
\textbf{Developmental staging}: Like human development, AI training should progress through stages, with early phases focused on value formation and basic understanding before advancing to capability development.
...
Training exclusively on internet text creates the AI equivalent of a child who only learns from books.
...
Similarly, AI systems should be designed for interpretability through behaviour and explanation, not just internal inspection.
...
\textbf{Collaborative development}: The current model of companies developing AI in secret, then deploying to users, resembles raising a child in isolation then expecting them to integrate into society.
...
\textbf{Accountability structures}: Instead of trying to assign blame when AI systems cause harm, we need frameworks that recognise the distributed nature of responsibility---developers, trainers, deployers, and the systems themselves all play roles.
...
International frameworks should focus not on preventing AI development but on ensuring it happens responsibly, with shared standards for healthy cognitive development.
...
\begin{enumerate}
\item \textbf{Reframe internal language}: Stop talking about ``using'' or ``deploying'' AI and start talking about ``collaborating with'' and ``developing'' brain-children.
