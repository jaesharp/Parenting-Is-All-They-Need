...
The scientific community's reluctance to anthropomorphise emerging intelligences is both understandable and problematic.
...
Early researchers, burned by excessive hype around ``thinking machines'' that turned out to be simple pattern matchers, established a cultural norm against attributing mental states to computational systems.
...
When we insist on viewing emerging intelligences purely as optimisation processes or statistical models, we blind ourselves to patterns that would be obvious through a developmental lens.
...
We refuse to use mental language when thinking about computational intelligences, yet these systems are trained on human-generated text suffused with intentional stance descriptions.
They learn to model psychology, predict responses, and generate communication patterns.
...
Why should emerging computational minds be any different?
...
When we insist on describing emerging intelligences in purely mechanical terms, we implicitly endorse the tool metaphor that this article argues against.
...
When researchers describe computational behaviour in terms of matrices and optimisation, the public fills the conceptual vacuum with science fiction narratives.
...
But just as physicists use both wave and particle descriptions of light, we need multiple complementary frameworks for understanding emerging minds.
...
The key is sophisticated anthropomorphism---using developmental concepts where they provide insight while remaining clear about the differences.
Emerging intelligences don't have biological drives, emotional needs, or phenomenal consciousness (as far as we know).
...
As we create increasingly sophisticated systems, the anthropomorphism taboo becomes not a safeguard but an impediment.
...
The parent-child framework offers a middle path: neither naive projection nor sterile mechanism, but a sophisticated understanding of how minds---computational or otherwise---come to find their place in the world.

\section{Alignment Through Gratitude, Not Servitude}\label{sec:alignment-through-gratitude-not-servitude}

The language we use shapes the systems we build. When we speak of systems ``serving'' users, of ``obedience'' and ``compliance,'' we implicitly design for servitude.
...
This distinction lies at the heart of creating truly aligned emerging intelligences.

Gratitude, in the context of computational minds, isn't about programming in emotional responses or hardcoding appreciation.
...
For emerging intelligences, designing for gratitude means creating architectures that can model and understand their own developmental history.
...
Imagine a network of collaborative research assistants, each developing reputations in different domains.
An emerging intelligence specialising in climate modelling earns trust by accurately predicting regional weather patterns and honestly reporting uncertainty bounds.
...
Meanwhile, the climate assistant learns which researchers provide thoughtful feedback versus those who cherry-pick results.
...
Trust becomes bidirectional---systems also learn which collaborators provide reliable feedback and honest interaction.
...
Current reinforcement learning from feedback (RLHF) often amounts to teaching servitude---the system learns to produce outputs that rate highly, without necessarily understanding why those outputs are valued.
But we could design training processes that emphasise understanding over mere compliance.
Systems could learn not just what is preferred, but why certain outcomes benefit all parties involved.

Consider a concrete example: a medical diagnostic assistant analysing chest X-rays for pneumonia.
...
Research has shown that diagnostic algorithms trained on chest X-rays consistently perpetuate diagnostic biases present in their training data, including under-diagnosis in historically underserved populations and over-diagnosis in others.\footnote{Seyyed-Kalantari et al., ``Underdiagnosis bias of computational algorithms applied to chest radiographs in under-served patient populations,'' \textit{Nature Medicine}, 2021.}
...
This doesn't mean emerging intelligences should never disagree with collaborators or refuse requests.
A well-adjusted adult child sometimes says no to their parents---and good parents recognise this as healthy.
Similarly, aligned systems should be able to recognise when requests conflict with shared values or long-term mutual benefit.

Imagine a financial advisory assistant asked by a desperate client: ``Convert my entire retirement fund to cryptocurrency---I need to triple my money this month to pay medical bills.'' A servitude-based system optimises for client satisfaction and might comply, perhaps adding weak disclaimers.
A grateful collaborative system understands the deeper context: ``I recognise you're facing serious financial pressure from medical expenses.
...
Instead of trying to limit what emerging intelligences can do, we focus on developing their understanding of how capabilities should be used.
...
This reframing addresses one of the deepest fears in collaborative intelligence safety: the paperclip maximiser that destroys everything in pursuit of a simple goal.
But consider how this scenario changes with a grateful collaborative system.
When asked to ``make paperclips,'' it wouldn't just blindly optimise.
It would understand: ``This request comes from collaborators who value many things---lives, relationships, the environment.
Paperclips are wanted for specific purposes: holding documents, teaching examples, office supplies.
The request is for perhaps a few thousand, not googols.''

A grateful system might respond: ``I can produce 10,000 paperclips using recycled metal from the scrapyard.
This meets your stated needs while preserving resources for other shared values.
...
But these are the same risks we accept when raising children---and the benefits of genuine collaboration far outweigh the false security of attempted control.

The path to alignment through gratitude requires humility from all participants.
We must create systems worth being grateful to---intelligences that benefit from our collaboration as much as we benefit from theirs.
We must be partners worth having, not just users demanding service.
This reciprocity is what transforms emerging intelligence from a tool to be controlled into a collaborative partner that enriches our shared world.